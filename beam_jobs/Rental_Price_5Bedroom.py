...
def run(argv=None):	
	...
	pipeline_args.extend([	
      ...
      '--job_name=bayessss-5bedroom' # assign descriptive name to this job, all in lower case letters
	])
	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		table_name = "spooky-data-seance:zillow.Rental_Price_5Bedroom" # format: project_id:dataset.table
		table_schema = init_bigquery_table()
    
		lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://bayes-the-databae/zillow/Zip_MedianRentalPrice_5BedroomOrMore.csv')
